...
class OneroA1SingleArm(Robot):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        try:
            ...
            if not self._arm.is_hardware_connected():
                raise RuntimeError(f"Failed to connect to hardware on {self.config.port}")
            ...
            # Only set connected after everything succeeds
    ...
    def get_observation(self) -> RobotObservation:
        ...
        if state is None:
            logger.warning("Failed to get arm state")
            # Return zeros
            return {f"{name}.pos": 0.0 for name in self.config.joint_names}

        obs = {}
        for i, joint_name in enumerate(self.config.joint_names):
            obs[f"{joint_name}.pos"] = float(state.positions[i])
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        for joint_name in self.config.joint_names:
            ...
            if self.config.enable_safety:
                ...
                if state is not None:
                    current = state.positions[self.config.joint_names.index(joint_name)]
                else:
                    current = value  # Skip clamping if state unavailable
                    delta = value - current
        ...
        if self._mit_mode_enabled:
            ...
            ret = self._arm.movej(
                ...
                trajectory_connect=1,  # Enable trajectory smoothing
            )
...
class OneroA1(Robot):
    ...
    def connect(self, calibrate: bool = True, enable_mit_mode: bool = False) -> None:
        """Connect to both arms.

        Args:
            calibrate: Whether to calibrate arms
            enable_mit_mode: If True, enables MIT control mode for both arms.
                           Master arm: zero-force (gravity compensation only)
                           Slave arm: high-stiffness position tracking
        """
        ...
        # 启用MIT模式
        if enable_mit_mode:
            logger.info("Enabling MIT control mode...")
            self._enable_mit_control()

    def _enable_mit_control(self) -> None:
        """启用MIT控制模式"""
        # 主臂：低增益零力控制
        self.master_arm._mit_kp = [0.0] * 7
        self.master_arm._mit_kd = [0.0] * 7
        self.master_arm.enable_mit_mode(True)

        # 从臂：高增益位置跟踪
        self.slave_arm.enable_mit_mode(True)

        logger.info("MIT control mode enabled for both arms")
    ...
    def master_slave_follow(self, mirror: bool | None = None) -> RobotAction:
        ...
        for i, key in enumerate(self.config.master_arm_config.joint_names):
            # Single arm observations don't have prefix, e.g. "joint1.pos"
            master_positions.append(master_obs[f"{key}.pos"])
        ...
        sent_action = {}
        if slave_action:
            if self.slave_arm._mit_mode_enabled:
                # MIT模式：直接更新目标位置
                raw_slave_positions = [slave_action[f"slave_{key}.pos"]
                                     for key in self.config.slave_arm_config.joint_names]
                self.slave_arm.set_mit_target(raw_slave_positions)
            else:
                # 传统movej模式
